...
module.exports = (() => {
	...
	const config = {
		info: {
			...
			github: "https://github.com/SuperNovaCollision/BetterDiscordPlugins/blob/main/MemeSounds/MemeSounds.plugin.js",
			github_raw: "https://raw.githubusercontent.com/SuperNovaCollision/BetterDiscordPlugins/main/MemeSounds/MemeSounds.plugin.js"
		},
		/* Settings */
		defaultConfig: [{
			/* General Settings */
							...
							if (!optimistic && lastMessageID != message.id) {
								...
								for (let sound of [...queue.entries()].sort((a, b) => a[0] - b[0])) {
									if (this.settings.toggle[sound[1].file.replace(/\..+$/, "")]) {
										let audio = new Audio("https://github.com/SuperNovaCollision/BetterDiscordPlugins/main/MemeSounds/MemeSounds/Sounds/" + sound[1].file);
